...
static u8 * format_my_plugin_trace (u8 * s, va_list * args)
{
  ...
  s = format (s, "MY_PLUGIN: sw_if_index %d, next index %d\n",
...
#define foreach_my_plugin_error \
_(SWAPPED, "Mac swap packets processed")

typedef enum 
{
#define _(sym,str) MY_PLUGIN_ERROR_##sym,
  foreach_my_plugin_error
#undef _
  MY_PLUGIN_N_ERROR,
} my_plugin_error_t;

#ifndef CLIB_MARCH_VARIANT
static char * my_plugin_error_strings[] = 
{
...
always_inline uword
my_plugin_inline (vlib_main_t * vm,
...
VLIB_NODE_FN (my_plugin_node) (vlib_main_t * vm, vlib_node_runtime_t * node,
                             vlib_frame_t * frame)
{
  if (PREDICT_FALSE (node->flags & VLIB_NODE_FLAG_TRACE))
    return my_plugin_inline (vm, node, frame, 1 /* is_ip4 */ ,
			    1 /* is_trace */ );
  else
    return my_plugin_inline (vm, node, frame, 1 /* is_ip4 */ ,
			    0 /* is_trace */ );
...
VLIB_REGISTER_NODE (my_plugin_node) = 
{
  ...
  .n_errors = ARRAY_LEN(my_plugin_error_strings),
